...
def _generate_reorder_advice(phrases, min_len=2):
    ...
    for ph in phrases:
        sspan, tspan, _, _ = ph
        if sspan[1] - sspan[0] >= min_len or tspan[1] - tspan[
            0] >= min_len:  # at least one phrase longer than min length
            legal_phs.append(ph)
...
# en_lines = [l.strip() for l in open("train.32k.en").readlines()]
# fr_lines = [l.strip() for l in open("train.32k.de").readlines()]
...
parser.add_argument('--en_lines-path', type=str,
                    help='path to train.32k.en file')
parser.add_argument('--fr_lines-path', type=str,
                    help='path to train.32k.de file')
...
en_lines = [l.strip() for l in open(args.en_lines_path).readlines()]
fr_lines = [l.strip() for l in open(args.fr_lines_path).readlines()]
...
# with open("phrases", "rb") as fp:  # Unpickling
