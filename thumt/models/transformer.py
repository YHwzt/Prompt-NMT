...
class TransformerDecoderLayer(modules.Module):
    "解码层"
    "Decoder包括self-attn, encdec-attn, 和feed forward "
...
class TransformerDecoderAdAttnLayer(modules.Module):
    "添加了prompt的Decoder包括self-attn, encdec-attn, advice_attn和feed forward "
    ...
    def __call__(self, x, attn_bias, encdec_bias, encdec_memory, state=None):
        ...
        # (3) Prompt Attention.每个解码器层中的交叉注意模块之后使用专用的提示注意子层
...
class Transformer(modules.Module):
    ...
    def __init__(self, params, name="transformer"):
        ...
        self.params = params
        # 将不同的对象及操作放在由name_scope()指定的区域中，便于在tensorboard中展示清晰的逻辑关系图
        with utils.scope(name):
...
class Phase1Transformer(Transformer):
    ...
    def encode(self, features, state):
        ...
        # 源语言和提词器的位置串联合并编码输出
        # B中值和query和key的相对位置有关系。
        # 相对位置一致的query - key pair，会采用相同的bias。
