...
from thumt.tokenizers import Tokenizer, AdviceTokenizer
...
class Dataset(IterableDataset):
    ...
    def tokenize(self, tokenizer: Tokenizer, bos: bytes = b"<bos>",
                 eos: bytes = b"<eos>", 
                 seed: int = 1) -> "TokenizedLineDataset":
        if isinstance(tokenizer, AdviceTokenizer):
            return TokenizedAdviceDataset(self, tokenizer, bos, eos, seed)
        else: 
...
class TokenizedAdviceDataset(Dataset):
    ...
    def __init__(self, dataset: TextLineDataset, tokenizer: Tokenizer,
        ...
        if elem_spec.elem_type is not bytes or elem_spec.shape != "[]":
            raise ValueError("TokenizedAdviceDataset only accepts a dataset with"
                              " ElementSpec(bytes, '[None]')")
        ...
        self._eos = eos
        self._spec = ElementSpec(List[bytes], "[None]")
        super(TokenizedAdviceDataset, self).__init__()
        # Inter State
        self.seed = seed
    ...
    def __iter__(self) -> Iterator:
        global epoch
        epoch += 1
        return Iterator(self, seed=self.seed, epoch=epoch)

    def __repr__(self) -> str:
        return "<TokenizedAdviceDataset:%s>" % self._dataset

    def _inputs(self) -> List[Dataset]:
        return [self._dataset]

    def copy(self) -> "TokenizedAdviceDataset":
        return TokenizedAdviceDataset(self._dataset.copy(), self._tokenizer,
                                    self._bos, self._eos)

    @property
    def name(self) -> str:
        return "TokenizedAdviceDataset"
